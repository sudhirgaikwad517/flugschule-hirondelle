...
// Same "data only, layout stays" idea as TeamContentEditor.tsx / the
// Ausbildung/Performance sub-page editors, for the /reisen/savoye-tour page
// (SavoyeTour.tsx). Gallery images are managed separately via Komponenten >
// Galerie, not here.
...
export const SavoyeTourContentEditor = () => {
  const notify = useNotify();
  const navigate = useNavigate();
  const { contentId } = useParams<{ contentId?: string }>();
  const id = contentId || 'savoye-tour';
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [content, setContent] = useState<SavoyeTourData | null>(null);
  ...
  const previewPath = `/${(!contentId && primaryMeta?.slug) || (id === 'savoye-tour' ? 'reisen/savoye-tour' : id)}`;
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      contentId ? Promise.resolve(null) : fetch('/api/fixed-page-settings/savoye-tour', { headers: authHeaders() }),
    ])
    ...
    fetch(`/api/sitepagecontent/${id}`, {
      ...
      .then(async () => {
        if (dupMeta) {
          ...
          if (updated.slug !== id) navigate(`/admin/savoye-tour-content/${updated.slug}`, { replace: true });
          else setDupMeta(updated);
        }
        if (primaryMeta) {
          const res = await fetch('/api/fixed-page-settings/savoye-tour', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', ...authHeaders() },
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="h5">Savoyer Alpentour - Inhalte</Typography>
      ...
      <Typography variant="body2" sx={{ color: '#666', mb: 3 }}>
        Nur Texte, Preise und Links der Seite /reisen/savoye-tour - das Design/Layout bleibt exakt wie es ist. Die Bildergalerie wird separat über Komponenten &gt; Galerie verwaltet.
      </Typography>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12, sm: 6 }}><TextField label="Titelbild Alt-Text" fullWidth value={content.heroAlt} onChange={(e) => setContent({ ...content, heroAlt: e.target.value })} /></Grid>
          </Grid>
        </CardContent>
      </Card>
          ...
          {content.contentBlocks.map((block, i) => (
            <Box key={i} sx={{ mb: 3, pb: 3, borderBottom: i < content.contentBlocks.length - 1 ? '1px solid #eee' : 'none' }}>
              <Grid container spacing={2}>
                <Grid size={{ xs: 12 }}>
                  <TextField
                    label="Überschrift (leer lassen für einen Absatz ohne Überschrift)"
                    fullWidth
                    value={block.heading}
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <TextField
            ...
            fullWidth
            value={content.leistungenHeading}
            onChange={(e) => setContent({ ...content, leistungenHeading: e.target.value })}
            sx={{ mb: 2 }}
          />
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          <Typography variant="h6" sx={{ mb: 2 }}>Badges (farbige Balken oben in der Seitenleiste)</Typography>
