...
// /infos/medien page (Medien.tsx) - two YouTube playlist embeds.

interface MedienData {
  heading: string;
  leftHeading: string;
  leftVideoUrl: string;
  rightHeading: string;
...
export const MedienContentEditor = () => {
  const notify = useNotify();
  const navigate = useNavigate();
  const { contentId } = useParams<{ contentId?: string }>();
  const id = contentId || 'medien';
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [content, setContent] = useState<MedienData | null>(null);
  ...
  const previewPath = `/${(!contentId && primaryMeta?.slug) || (id === 'medien' ? 'infos/medien' : id)}`;
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      contentId ? Promise.resolve(null) : fetch('/api/fixed-page-settings/medien', { headers: authHeaders() }),
    ])
    ...
    fetch(`/api/sitepagecontent/${id}`, {
      ...
      .then(async () => {
        if (dupMeta) {
          ...
          if (updated.slug !== id) navigate(`/admin/medien-content/${updated.slug}`, { replace: true });
          else setDupMeta(updated);
        }
        if (primaryMeta) {
          const res = await fetch('/api/fixed-page-settings/medien', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', ...authHeaders() },
  ...
  const set = (field: keyof MedienData) => (e: React.ChangeEvent<HTMLInputElement>) => setContent({ ...content, [field]: e.target.value });

  return (
    <Box sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="h5">Medien - Inhalte</Typography>
      ...
      <Typography variant="body2" sx={{ color: '#666', mb: 3 }}>
        Nur Texte und Video-Links der Seite /infos/medien - das Design/Layout bleibt exakt wie es ist.
      </Typography>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          <Typography variant="h6" sx={{ mb: 2 }}>Linke Spalte</Typography>
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, sm: 6 }}><TextField label="Überschrift" fullWidth value={content.leftHeading} onChange={set('leftHeading')} /></Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField
                label="Video-URL"
                fullWidth
                value={content.leftVideoUrl}
                onChange={set('leftVideoUrl')}
                helperText="YouTube Playlist-Embed-URL, z.B. https://www.youtube.com/embed/videoseries?list=..."
              />
            </Grid>
          </Grid>
        </CardContent>
      ...
      <Card>
        <CardContent>
          <Typography variant="h6" sx={{ mb: 2 }}>Rechte Spalte</Typography>
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, sm: 6 }}><TextField label="Überschrift" fullWidth value={content.rightHeading} onChange={set('rightHeading')} /></Grid>
            <Grid size={{ xs: 12, sm: 6 }}>
              <TextField
                label="Video-URL"
                fullWidth
                value={content.rightVideoUrl}
                onChange={set('rightVideoUrl')}
                helperText="YouTube Playlist-Embed-URL, z.B. https://www.youtube.com/embed/videoseries?list=..."
              />
            </Grid>
          </Grid>
