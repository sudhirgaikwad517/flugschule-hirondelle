...
export const BergamoTourContentEditor = () => {
  const notify = useNotify();
  const navigate = useNavigate();
  const { contentId } = useParams<{ contentId?: string }>();
  const id = contentId || 'bergamo-tour';
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [content, setContent] = useState<BergamoTourData | null>(null);
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      contentId ? Promise.resolve(null) : fetch('/api/fixed-page-settings/bergamo-tour', { headers: authHeaders() }),
    ])
    ...
    fetch(`/api/sitepagecontent/${id}`, {
      ...
      .then(async () => {
        if (dupMeta) {
          ...
          if (updated.slug !== id) navigate(`/admin/bergamo-tour-content/${updated.slug}`, { replace: true });
          else setDupMeta(updated);
        }
        if (primaryMeta) {
          const res = await fetch('/api/fixed-page-settings/bergamo-tour', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', ...authHeaders() },
  ...
  const addBlock = () => setContent({ ...content, contentBlocks: [...content.contentBlocks, { heading: '', text: '' }] });
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="h5">Bergamo-Tour - Inhalte</Typography>
      ...
      <Typography variant="body2" sx={{ color: '#666', mb: 3 }}>
        Nur Texte und Preise der Seite /reisen/bergamo-tour - das Design/Layout bleibt exakt wie es ist. Die Bildergalerie wird separat über Komponenten &gt; Galerie verwaltet.
      </Typography>
        ...
        <CardContent>
          ...
          {content.contentBlocks.map((block, i) => (
            <Box key={i} sx={{ mb: 3, pb: 3, borderBottom: i < content.contentBlocks.length - 1 ? '1px solid #eee' : 'none' }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                <Typography variant="subtitle2" sx={{ color: '#666' }}>{block.heading || `Abschnitt ${i + 1}`}</Typography>
              ...
              <TextField label="Text" fullWidth multiline minRows={3} value={block.text} onChange={(e) => updateBlock(i, 'text', e.target.value)} />
            </Box>
          ))}
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          <TextField label="Überschrift Leistungen" fullWidth value={content.leistungenHeading} onChange={(e) => setContent({ ...content, leistungenHeading: e.target.value })} sx={{ mb: 2 }} />
            ...
            <Grid container spacing={2} key={i} sx={{ mb: 1.5, alignItems: 'center' }}>
              <Grid size={{ xs: 10 }}>
                <TextField
                  ...
                  multiline
                  value={item}
                  onChange={(e) => updateLeistung(i, e.target.value)}
                  helperText="Zeilenumbruch mit Enter möglich"
                />
              </Grid>
              <Grid size={{ xs: 2 }}>
                <IconButton onClick={() => removeLeistung(i)} title="Zeile löschen">
                  <DeleteIcon fontSize="small" color="error" />
                </IconButton>
              </Grid>
            </Grid>
        ...
        <CardContent>
          ...
          {content.badges.map((badge, i) => (
            <Grid container spacing={2} key={i} sx={{ mb: 1.5, alignItems: 'center' }}>
              ...
              <Grid size={{ xs: 10, sm: 4 }}><TextField label="Farbe (Hex)" fullWidth value={badge.color} onChange={(e) => updateBadge(i, 'color', e.target.value)} /></Grid>
              <Grid size={{ xs: 2, sm: 1 }}>
