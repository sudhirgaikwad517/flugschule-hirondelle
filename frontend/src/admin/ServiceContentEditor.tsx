...
// /service page (Service.tsx) and its 4 fixed items.

interface ServiceItem { id: string; title: string; description: string; image: string; link: string }
interface ServiceData { items: ServiceItem[] }
...
export const ServiceContentEditor = () => {
  const notify = useNotify();
  const navigate = useNavigate();
  const { contentId } = useParams<{ contentId?: string }>();
  const id = contentId || 'service';
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      contentId ? Promise.resolve(null) : fetch('/api/fixed-page-settings/service', { headers: authHeaders() }),
    ])
    ...
    fetch(`/api/sitepagecontent/${id}`, {
      ...
      .then(async () => {
        if (dupMeta) {
          ...
          if (updated.slug !== id) navigate(`/admin/service-content/${updated.slug}`, { replace: true });
          else setDupMeta(updated);
        }
        if (primaryMeta) {
          const res = await fetch('/api/fixed-page-settings/service', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', ...authHeaders() },
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="h5">Service - Inhalte</Typography>
      ...
      <Typography variant="body2" sx={{ color: '#666', mb: 3 }}>
        Nur Texte und Bilder der Seite /service - das Design/Layout bleibt exakt wie es ist.
      </Typography>
          ...
          <Grid container spacing={3}>
            {content.items.map((item, i) => (
              <Grid key={i} size={{ xs: 12, md: 6 }}>
                <Typography variant="subtitle2" sx={{ mb: 1, color: '#666' }}>{item.title || `Punkt ${i + 1}`}</Typography>
                <ImageSlot url={item.image} onUploaded={(url) => updateItem(i, 'image', url)} label="Bild ersetzen" />
                <TextField label="Titel" fullWidth value={item.title} onChange={(e) => updateItem(i, 'title', e.target.value)} sx={{ mb: 2 }} />
                <TextField label="Beschreibung" fullWidth multiline minRows={3} value={item.description} onChange={(e) => updateItem(i, 'description', e.target.value)} sx={{ mb: 2 }} />
                <TextField label='Link für den "WEITERLESEN"-Button' fullWidth value={item.link} onChange={(e) => updateItem(i, 'link', e.target.value)} />
              </Grid>
            ))}
          </Grid>
        </CardContent>
