...
export const PerformanceContentEditor = () => {
  const notify = useNotify();
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="h5">Performance - Inhalte</Typography>
      ...
      <Typography variant="body2" sx={{ color: '#666', mb: 3 }}>
        Nur Texte und Bilder der Seite /performance - das Design/Layout bleibt exakt wie es ist.
      </Typography>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <TextField label="Zitat" fullWidth multiline minRows={2} value={content.introQuote} onChange={(e) => setContent({ ...content, introQuote: e.target.value })} />
        </CardContent>
      </Card>

      <Card sx={{ mb: 3 }}>
        <CardContent>
          <Typography variant="h6" sx={{ mb: 2 }}>Die 6 farbigen Kacheln</Typography>
          <Grid container spacing={2}>
            {content.badges.map((badge, i) => (
              <Grid key={i} size={{ xs: 12, sm: 6, md: 4 }}>
                <TextField label={`Kachel ${i + 1}`} fullWidth value={badge.label} onChange={(e) => updateBadge(i, e.target.value)} />
              </Grid>
            ))}
          </Grid>
        </CardContent>
