...
// /infos/gelaende/bad-kreuznach's admin editor - rebuilt from a single raw
// HTML textarea into real fields, same "one field per line, Card per
// section" convention used everywhere else this session. See
// BadKreuznach.tsx for the matching public page.
...
export const BadKreuznachContentEditor = () => {
  const notify = useNotify();
  const navigate = useNavigate();
  const { contentId } = useParams<{ contentId?: string }>();
  const id = contentId || 'bad-kreuznach';
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [content, setContent] = useState<BadKreuznachData | null>(null);
  ...
  const previewPath = `/${(!contentId && primaryMeta?.slug) || (id === 'bad-kreuznach' ? 'infos/gelaende/bad-kreuznach' : id)}`;
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      contentId ? Promise.resolve(null) : fetch('/api/fixed-page-settings/bad-kreuznach', { headers: authHeaders() }),
    ])
    ...
    fetch(`/api/sitepagecontent/${id}`, {
      ...
      .then(async () => {
        if (dupMeta) {
          ...
          if (updated.slug !== id) navigate(`/admin/bad-kreuznach-content/${updated.slug}`, { replace: true });
          else setDupMeta(updated);
        }
        if (primaryMeta) {
          const res = await fetch('/api/fixed-page-settings/bad-kreuznach', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', ...authHeaders() },
  ...
  const updateEckdatenRow = (i: number, field: keyof EckdatenRow, value: string) => {
    const eckdatenRows = [...content.eckdatenRows];
    eckdatenRows[i] = { ...eckdatenRows[i], [field]: value };
    setContent({ ...content, eckdatenRows });
  };
  const addEckdatenRow = () => setContent({ ...content, eckdatenRows: [...content.eckdatenRows, { label: '', value: '' }] });
  const removeEckdatenRow = (i: number) => setContent({ ...content, eckdatenRows: content.eckdatenRows.filter((_, idx) => idx !== i) });

  return (
    <Box sx={{ p: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="h5">Bad Kreuznach - Inhalte</Typography>
      ...
      <Typography variant="body2" sx={{ color: '#666', mb: 3 }}>
        Nur Texte und Bilder der Seite {previewPath} - das Design/Layout bleibt exakt wie es ist.
      </Typography>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <TextField
            ...
            helperText='Erscheint als eigene Überschrift über Text/Karte, z.B. "Schleppgelände..."'
          />
        </CardContent>
      </Card>
        ...
        <CardContent>
          ...
          {content.eckdatenRows.map((row, i) => (
            <Grid container spacing={2} key={i} sx={{ mb: 1.5, alignItems: 'center' }}>
              <Grid size={{ xs: 12, sm: 4 }}><TextField label="Bezeichnung" fullWidth value={row.label} onChange={(e) => updateEckdatenRow(i, 'label', e.target.value)} /></Grid>
              <Grid size={{ xs: 10, sm: 7 }}><TextField label="Wert" fullWidth value={row.value} onChange={(e) => updateEckdatenRow(i, 'value', e.target.value)} /></Grid>
              <Grid size={{ xs: 2, sm: 1 }}>
                <IconButton onClick={() => removeEckdatenRow(i)} title="Zeile löschen">
                  <DeleteIcon fontSize="small" color="error" />
                </IconButton>
              </Grid>
            </Grid>
          ))}
          <Button startIcon={<AddIcon />} onClick={addEckdatenRow} sx={{ mt: 1 }}>Zeile hinzufügen</Button>
        </CardContent>
      </Card>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          <Typography variant="h6" sx={{ mb: 2 }}>Bilder</Typography>
          <Typography variant="body2" sx={{ mb: 1, color: '#666' }}>Hauptbild</Typography>
          <ImageSlot url={content.mainImage} onUploaded={(url) => setContent({ ...content, mainImage: url })} label="Bild ersetzen" />
          <Typography variant="body2" sx={{ mb: 1, color: '#666' }}>Zweites Bild (Anfahrtsskizze)</Typography>
          <ImageSlot url={content.routeImage} onUploaded={(url) => setContent({ ...content, routeImage: url })} label="Bild ersetzen" />
        </CardContent>
      </Card>

      <Card sx={{ mb: 3 }}>
        <CardContent>
          <Typography variant="h6" sx={{ mb: 2 }}>Anfahrt</Typography>
          <TextField label="Überschrift" fullWidth value={content.anfahrtHeading} onChange={(e) => setContent({ ...content, anfahrtHeading: e.target.value })} sx={{ mb: 2 }} />
          <TextField label="Text" fullWidth multiline minRows={2} value={content.anfahrtText} onChange={(e) => setContent({ ...content, anfahrtText: e.target.value })} sx={{ mb: 2 }} />
          <TextField label="Parkplatz-Hinweis" fullWidth multiline minRows={2} value={content.parkplatzText} onChange={(e) => setContent({ ...content, parkplatzText: e.target.value })} />
        </CardContent>
      </Card>
