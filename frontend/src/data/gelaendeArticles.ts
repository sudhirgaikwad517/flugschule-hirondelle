// Verbatim content transcribed from the old Joomla site's hiron_content
// table (catid=2, the Gelände articles). Hardcoded rather than fetched
// from an API because this reference content (GPS coords, hazard notes,
// driving directions) is effectively static and the goal here is a
// 100%-accurate one-time port of the old site's article text, not
// ongoing CMS-editability.
export interface GelaendeArticle {
  slug: string;
  title: string;
  html: string;
}

export const GELAENDE_ARTICLES: GelaendeArticle[] = [
  {
    slug: 'billings',
    title: 'Billings',
...
<div class="col-md-6"><a href="http://maps.apple.com/?q=%2049.754684, 8.794765" target="_blank" rel="alternate noopener noreferrer" title="Öffnet Google Maps App"><img src="/images/Google_Routenplaner_Maps.gif" alt="" width="120" height="90" /> </a></div>
...
export const getGelaendeArticleBySlug = (slug: string) =>
  GELAENDE_ARTICLES.find((a) => a.slug === slug);
