...
// Fallbacks match the page's current live copy exactly (see
// Admin > Seiten > Team, backend `team` SitePageContent kind).
const DEFAULT_CONTENT: TeamData = {
  eyebrow: 'INFOS',
...
export const Team = ({ contentId }: { contentId?: string } = {}) => {
  const [content, setContent] = useState<TeamData>(DEFAULT_CONTENT);

  useEffect(() => {
    fetch(`/api/sitepagecontent/public/${contentId || 'team'}`)
    ...
    <div className="w-full bg-white font-luxurysans pb-20">
      ...
      <section className="pt-16 md:pt-24 pb-8 md:pb-12">
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-10">
                ...
                <div>
                  <h3 className="text-xl font-luxury text-luxury-dark italic mb-4">{content.ausbildungHeading}</h3>
                  <SafeHtml
                    className="text-[14px] text-gray-600 font-light leading-relaxed [&_a]:text-[#428bca] [&_a]:hover:text-[#2a6496] [&_a]:hover:underline [&_a]:font-medium"
                    html={content.ausbildungHtml}
                  />
                </div>

                {/* Reisen */}
                <div>
                  <h3 className="text-xl font-luxury text-luxury-dark italic mb-4">{content.reisenHeading}</h3>
                  <SafeHtml
                    className="text-[14px] text-gray-600 font-light leading-relaxed [&_a]:text-[#428bca] [&_a]:hover:text-[#2a6496] [&_a]:hover:underline [&_a]:font-medium"
                    html={content.reisenHtml}
                  />
                </div>
        ...
        <div className="container mx-auto px-4 lg:px-8 max-w-[1200px] pt-16 mt-8">
          <h2 className="text-2xl font-luxury text-luxury-dark uppercase mb-4">{content.shopHeading}</h2>

          <div className="lg:col-span-8">
            <SafeHtml
              className="text-[15px] text-gray-600 font-light leading-relaxed mb-10 max-w-4xl [&_a]:text-[#428bca] [&_a]:hover:text-[#2a6496] [&_a]:hover:underline [&_a]:font-medium"
              html={content.shopIntroHtml}
            />

            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-6">
              {content.shopBrands.map((brand, idx) => (
                <a
                  key={idx}
                  href={brand.url}
                  target="_blank"
